...
(async () => {
  ...
  const editable=state=>({title:state.title,content:state.content,tags:state.tags,status:state.status,priority:state.priority});
  const fields=()=>({title:$('title').value,content:editor.value,tags:[...new Set($('tags').value.split(',').map(s=>s.trim()).filter(Boolean))],status:$('entry-status').value,priority:$('priority').value?Number($('priority').value):null});
  ...
  function offerRecovery() {
    ...
    try {
      for(let i=0;i<localStorage.length;i++) {
        ...
        if(!key.startsWith(draftPrefix+(entry?.entry_id||'new')+':'))continue;
  ...
  async function detectConflict() {
    if(!entry)return;
    remote=await api('/api/entries/read?'+new URLSearchParams({entry_id:entry.entry_id,max_content_length:40000}));
  ...
  async function save() {
    ...
    } finally {busy=false;$('save').disabled=!canWrite;if(saved&&dirty()&&$('autosave').checked)timer=setTimeout(()=>save(),1800);}
  }
  $('save').onclick=save;
  ...
  async function listEntries(more=false) {
    ...
    for(const [key,id] of [['query','query'],['kind','kind-filter'],['status','status-filter']])if($(id).value)params[key]=$(id).value;
    const result=await api('/api/entries?'+new URLSearchParams(params));
    if(!more)$('entry-list').replaceChildren();
    for(const item of result.entries){
      const a=document.createElement('a');a.className='entry-row';a.href=prefix+'/entries/'+item.entry_id;
      a.textContent=item.title||item.slug||'Untitled';const small=document.createElement('small');
      small.textContent=`${item.kind} · v${item.revision} · ${new Date(item.modified_at).toLocaleString()}`;a.append(small);
  ...
  async function listHistory(more=false) {
    if(!entry)return;
    const result=await api('/api/entries/revisions?'+new URLSearchParams({entry_id:entry.entry_id,limit:25,offset:more?historyOffset:0}));
  ...
  async function compare(number) {
    ...
    const [version,result]=await Promise.all([
      api('/api/entries/read?'+new URLSearchParams({entry_id:id,revision:number,max_content_length:40000})),
      api('/api/entries/compare',{entry_id:id,revision:number,content:draft})]);
  ...
  $('rebase').onclick=()=>{
    ...
    revision=remote.revision;base=editable(remote.state);entry=remote;blocked=false;$('conflict').hidden=true;remote=null;changed();
  };
  $('take-remote').onclick=()=>{
