...
from . import claims
from .claim_schemas import ManageResource, OfferWork, ClaimWork, UpdateClaim, ReadClaim, ValidateClaim, GuardRunRequest
...
def register(mcp, endpoint):
    ...
    contracts=[('/claims/guard','POST',claims.record_guard_run,GuardRunRequest),('/resources/manage','POST',claims.manage_work_resource,ManageResource),
    ...
    return [Route('/api/inflight'+path,endpoint({method:(operation,schema)}),methods=[method]) for path,method,operation,schema in contracts]
